...
const painter4=canvas4.getContext('2d');

canvas4.width = canvas4.offsetWidth;
canvas4.height = canvas4.offsetHeight;
const divWidth1=canvas4.offsetWidth;
const divHeight1=canvas4.offsetHeight;
...
function drawCurve(beginX, beginY, ctrlX, ctrlY, endX, endY) {
  painter4.beginPath();
  painter4.moveTo(beginX, beginY);
  painter4.quadraticCurveTo(ctrlX, ctrlY, endX, endY);
  painter4.strokeStyle = "#432385";
  painter4.lineWidth = 3;
  painter4.stroke();
}
...
function setDot(timeBegin, timeEnd, beginX, beginY, ctrlX, ctrlY, endX, endY) {
  ...
  if(time<timeBegin) {
    ...
    painter4.fillStyle = "#F0F8FF	";
    x= endX;y= endY;
  }

  painter4.beginPath();
  painter4.arc(x, y, 7, 0, 2 * Math.PI);
  painter4.fill();
}
...
function setDotMarkSin(timeBegin, beginX, beginY, label) {
  if(time<timeBegin) {
    ...
    if(time%1000>=500)
      painter4.fillStyle = "#FF0000";
    else
      painter4.fillStyle = "#FFFFFF";
  }
  painter4.beginPath();
  painter4.arc(beginX, beginY, 12, 0, 2 * Math.PI);
  painter4.fill();

  painter4.fillStyle = "#FF0000"; // 设置文本颜色
  painter4.font = "bold 30px Arial"; // 设置文本字体和大小
  painter4.fillText(label, beginX - 15, beginY - 15); 
}

function set1() {
  painter4.clearRect(0, 0, divWidth1, divHeight1);
